...
const userModel = {
  ...
  getChats: async (chat: { receiver_id: string; sender_id: string }) => {
    try {
      const { data, error } = await supabase
        .from("chats")
        .select("*")
        .or(
          `and(sender_id.eq.${chat.receiver_id},receiver_id.eq.${chat.sender_id}),and(sender_id.eq.${chat.sender_id},receiver_id.eq.${chat.receiver_id})`
        );
  ...
  getLastMessages: async (chat: { receiverId: string; senderId: string }) => {
    try {
      ...
      const { data, error } = await supabase
        .from("chats")
        .select(
          "message,receiver_id(profile_picture_url,first_name,last_name,id),created_at"
        )
        .or(
          `and(sender_id.eq.${chatInSnakeCasing.receiver_id},receiver_id.eq.${chatInSnakeCasing.sender_id}),and(sender_id.eq.${chatInSnakeCasing.sender_id},receiver_id.eq.${chatInSnakeCasing.receiver_id})`
        )
        .order("created_at", {
          ascending: false,
        })
