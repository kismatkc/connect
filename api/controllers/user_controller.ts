...
const userController = {
  ...
  createMessages: async (req: Request, res: Response) => {
    try {
      const message: {
        message: string;
        senderId: string;
  ...
  getChats: async (req: Request, res: Response) => {
    try {
      const ids = req.query as { receiver_id: string; sender_id: string };
  ...
  getLastMessages: async (req: Request, res: Response) => {
    try {
      const ids = req.body as unknown as {
        senderId: string;
        receiverId: string;
      }[];
      console.log(ids);
      ...
      const responses = (await Promise.allSettled(
        ids.map(async (chat) => {
          const response = (await userModel.getLastMessages(
            chat
          )) as unknown as { status: string; value: any };

          return response;
        })
      )) as { status: string; value: any }[];
